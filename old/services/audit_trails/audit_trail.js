...
function processData(audit_trail, customerDict, rentPactDict, unitDict, adminDict, tenantUserDict) {
    ...
    if (before) {
        if (before.customer_id) {
            before['vcustname'] = customerDict[before.customer_id]; delete before.customer_id
        }
        if (before.rent_pact_id) {
            before['vpactnum'] = rentPactDict[before.rent_pact_id]; delete before.rent_pact_id
        }
        if (before.unit_id) {
            before['unitname'] = unitDict[before.unit_id]; delete before.unit_id
        }
    }
    if (after) {
        if (after.customer_id) {
            after['vcustname'] = customerDict[after.customer_id]; delete after.customer_id
        }
        if (after.rent_pact_id) {
            after['vpactnum'] = rentPactDict[after.rent_pact_id]; delete after.rent_pact_id
        }
        if (after.unit_id) {
            after['unitname'] = unitDict[after.unit_id]; delete after.unit_id
        }
    }
